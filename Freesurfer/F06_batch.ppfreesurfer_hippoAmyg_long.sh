...
if [ ! -e ${basedir}/${subID}/${session}.long.${subID}_base/mri/rh.amygNucVolumes-T1.long.v2x.txt ]; then
export SUBJECTS_DIR=${basedir}/${subID}
#rm ${basedir}/${subID}/${subID}_base/scripts/IsRunningLongHPsub.lh+rh
#recon-all -s freesurfer -i ${basedir}/${subID}/${session}/${fileanat} -all
segmentHA_T1_long.sh ${subID}_base
#recon-all -long ${session} ${subID}_base -hippocampal-subfields-T1 
